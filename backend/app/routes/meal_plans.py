...
def get_meal_plans_for_week(start_date: date, db: Session = Depends(get_db)):
...
def get_meal_plan(id: int, db: Session = Depends(get_db)):
...
def update_meal_plan(id: int, meal_plan: MealPlanUpdate, db: Session = Depends(get_db)):
